...
bool
op_source::get_operand (irange &r, tree expr)
{
  if (!gimple_range_ssa_p (expr))
    return get_tree_range (r, expr);

  // Otherwise look for a stmt first (which covers the other gori option too).
  if (m_stmt)
...
relation_kind
op_source::query_relation (tree op1, tree op2)
{   
  if (!m_query.oracle ())
    return VREL_NONE;
  if (m_edge)
    return m_query.query_relation (m_edge, op1, op2);
...
bool
fold_using_range::fold_stmt (irange &r, gimple *s, op_source &src, tree name)
{
  bool res = false;
...
bool
fold_using_range::range_of_range_op (irange &r, gimple *s, op_source &src)
{
  ...
  if (src.get_operand (range1, op1))
      ...
      if (!op2)
	{
	  ...
	  postfold_range_op (handler, s, lhs, r, op1, range1, src);
	  return true;
	}

      if (src.get_operand (range2, op2))
	{
	  ...
	  postfold_range_op (handler, s, lhs, r, op1, range1, op2, range2, src);
	  if (is_a<gcond *> (s))
...
// See if an && or || expression can be folded based on relations. ie
//   c_2 = a_6 > b_7
//   c_3 = a_6 < b_7
//   c_4 = c_2 && c_3
					...
					op_source &src)
{
  ...
  if (!src.m_gori || !src.m_query.oracle () || lhs_range.singleton_p ())
    return;
  ...
  tree ssa1_dep2 = src.m_gori->depend2 (ssa1);
  tree ssa2_dep2 = src.m_gori->depend2 (ssa2);
  if (!ssa1_dep2 || !ssa2_dep2)
    return;

  tree ssa1_dep1 = src.m_gori->depend1 (ssa1);
  tree ssa2_dep1 = src.m_gori->depend1 (ssa2);
...
bool
fold_using_range::range_of_address (irange &r, gimple *stmt, op_source &src)
{
...
bool
fold_using_range::range_of_call (irange &r, gcall *call, op_source &src)
{
...
bool
fold_using_range::range_of_cond_expr (irange &r, gassign *s, op_source &src)
{
...
bool
fold_using_range::range_of_phi (irange &r, gphi *phi, op_source &src)
{
...
void
fold_using_range::postfold_range_op (range_operator *handler, gimple *s,
				     tree lhs, irange &lhs_range,
				     tree op1, const irange &range1,
				     op_source &src)
{
  // Register a direct dependency betwene LHS and OP1 if appropriate.
  if (lhs && src.m_gori)
    src.m_gori->register_dependency (lhs, op1);

  if (!src.m_process || !src.m_query.oracle () || src.m_stmt != s) 
    return;
  ...
  if (lhs && gimple_range_ssa_p (op1))
	...
	src.m_query.oracle ()->register_relation (s, relation, lhs, op1);
    }
}
				     ...
				     op_source &src)
{
  ...
  // Don't process new relations if not requested.
  if (!src.m_query.oracle () || !src.m_process || src.m_stmt != s)
    return;
  ...
  if (lhs && ssa1)
    {
      ...
      if (relation != VREL_NONE)
	src.m_query.oracle ()->register_relation (s, relation, lhs, ssa1);
    }
  ...
  if (lhs && ssa2)
    {
      ...
      if (relation != VREL_NONE)
	src.m_query.oracle ()->register_relation (s, relation, lhs, ssa2);
    }
}

// If this is a conditional branch, register any outgoing edge relations.
void
fold_using_range::postfold_gcond_edges (gcond *s, op_source &src)
{
  int_range_max r;
  tree name;
  ...
  // Outgoing relations on a branch require dependency chains.
  if (!src.m_gori || !src.m_process)
    return;
  ...
  // First, register the gcond itself.
  tree ssa1 = gimple_range_ssa_p (gimple_range_operand1 (s));
  tree ssa2 = gimple_range_ssa_p (gimple_range_operand2 (s));
  if (ssa1 && ssa2)
    {
	  ...
	  if (relation != VREL_NONE)
	    src.m_query.oracle ()->register_relation (e0, relation, ssa1, ssa2);
	}
      if (e1)
        {
	  ...
	  relation_kind relation = handler->op1_op2_relation (r);
	  if (relation != VREL_NONE)
	    src.m_query.oracle ()->register_relation (e1, relation, ssa1, ssa2);
	}
    }
 
  // NOw look for other exports.,

  FOR_EACH_GORI_EXPORT_NAME (*(src.m_gori), bb, name)
    {
	...
	continue;
      ...
      if (ssa1 && ssa2)
        {
	  if (e0 && src.m_gori->outgoing_edge_range_p (r, e0, name)
	      && r.singleton_p ())
	    {
	      relation_kind relation = handler->op1_op2_relation (r);
	      if (relation != VREL_NONE)
		src.m_query.oracle ()->register_relation (e0, relation, ssa1,
							  ssa2);
	    }
	  if (e1 && src.m_gori->outgoing_edge_range_p (r, e1, name)
	      && r.singleton_p ())
	    {
	      relation_kind relation = handler->op1_op2_relation (r);
	      if (relation != VREL_NONE)
		src.m_query.oracle ()->register_relation (e1, relation, ssa1,
							  ssa2);
	    }
	}
    }
