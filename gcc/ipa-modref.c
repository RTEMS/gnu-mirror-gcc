...
process_fnspec (modref_summary *cur_summary, gcall *call, bool ignore_stores)
{
  ...
  if (!fnspec.known_p () || fnspec.barrier_p ())
    {
  ...
  if (!fnspec.stores_known_p ())
	...
	if (!fnspec.arg_specified_p (i))
	  ;
	else if (fnspec.arg_written_p (i))
	  {
	    modref_parm_map map = parm_map_for_arg (call, i);
      ...
      if (fnspec.clobbers_errno_p () && flag_errno_math)
	cur_summary->writes_errno = true;
    }
  return true;
}
...
modref_summaries::insert (struct cgraph_node *node, modref_summary *)
{
  ...
  /* If funtion is not in giple form yet or the pass is disabled, make
     sure we do not keep stale summary.  */
