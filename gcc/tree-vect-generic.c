...
expand_vector_condition (gimple_stmt_iterator *gsi)
{
  ...
  if (expand_vec_cond_expr_p (type, TREE_TYPE (a1), TREE_CODE (a)))
      ...
      if (a_is_comparison)
	{
      ...
      gcc_assert (TREE_CODE (a) == SSA_NAME);
    }
...
gimple_lower_vec_cond_expr (gimple_stmt_iterator *gsi)
{
  ...
  if (code == VEC_COND_EXPR)
	...
	{
	  gcc_assert (VECTOR_BOOLEAN_TYPE_P (TREE_TYPE (rhs1)));
	  if (get_vcond_mask_icode (mode, TYPE_MODE (TREE_TYPE (rhs1)))
	      != CODE_FOR_nothing)
	    {
	      gimple *g
		= gimple_build_call_internal (IFN_VCOND_MASK,
					      3, rhs1, rhs2, rhs3);
