...
run_gcc (unsigned argc, char *argv[])
{
  ...
  if (linker_output)
      ...
      else
	{
	  ...
	  if (!dumppfx)
	    dumppfx = xstrndup (dumppfx, obase - linker_output);
	  ...
	  if ((temp = strrchr (obase + 1, '.'))
	    ...
	    obase = xstrndup (obase, blen - xlen);
	  else
	    xlen = 0;
...

  {
    /* If there's no directory component in the dumppfx, add one, so
       that, when it is used as -dumpbase, it overriding any
       occurrence of -dumpdir that might have been passed
       automatically.  */
    if (lbasename (dumppfx) == dumppfx)
      {
...
cont1:
  ...
  if (lto_mode == LTO_MODE_LTO)
	...
	{
	  char *prefix = concat (linker_output_or_a, "-", NULL);
...
cont:
      ...
      for (i = 0; i < nr; ++i)
	{
	  ...
	  snprintf (dumpbase, dumpbase_len, "%sltrans%u", dumppfx, i);
	  argv_ptr[0] = dumpbase;
