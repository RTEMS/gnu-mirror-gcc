...
vec_safe_grow (vec<T, A, vl_embed> *&v, unsigned len,
	       bool exact = true CXX_MEM_STAT_INFO)
{
  ...
  if (exact)
    ...
    vec_safe_reserve (v, len - oldlen PASS_MEM_STAT);
  v->quick_grow (len);
}
...
vec_safe_grow_cleared (vec<T, va_heap, vl_ptr> *&v,
		       unsigned len, bool exact = true CXX_MEM_STAT_INFO)
{
...
public:
  ...
  T &pop (void);
  void truncate (unsigned);
  void safe_grow (unsigned, bool = true CXX_MEM_STAT_INFO);
  void safe_grow_cleared (unsigned, bool = true CXX_MEM_STAT_INFO);
...
vec<T, va_heap, vl_ptr>::safe_grow (unsigned len, bool exact MEM_STAT_DECL)
{
  ...
  if (exact)
    ...
    reserve (len - oldlen PASS_MEM_STAT);
  if (m_vec)
...
template<typename T>
inline void
vec<T, va_heap, vl_ptr>::safe_grow_cleared (unsigned len, bool exact MEM_STAT_DECL)
{
  unsigned oldlen = length ();
  size_t growby = len - oldlen;
