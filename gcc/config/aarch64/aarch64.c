			 ...
			 tree function)
{
  ...
  if (vcall_offset == 0)
    aarch64_add_offset (Pmode, this_rtx, this_rtx, delta, p_temp1, p_temp0, false);
  else
    {
      ...
      else
	{
      ...
      if (POmode == noncapability_mode (ptr_mode))
	aarch64_emit_move (temp1, gen_rtx_MEM (POmode, addr));
      else
	{
	  /* ILP32: no capabilities here.  */
	  gcc_assert (Pmode == POmode);
	  aarch64_emit_move (p_temp1,
			     gen_rtx_SIGN_EXTEND (Pmode,
						  gen_rtx_MEM (ptr_mode, addr)));
	}

      check_emit_insn (gen_add2_insn (this_rtx, temp1));
    }
