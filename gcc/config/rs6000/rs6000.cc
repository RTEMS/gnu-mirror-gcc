...
rs6000_hard_regno_mode_ok_uncached (int regno, machine_mode mode)
{
  ...
  /* On ISA 3.1 (power10), MMA accumulator modes need FPR registers divisible
     ...
     XOmode in VSX registers only needs to be in registers divisible by 2 to
     allow using load vector pair and store vector pair to load/store the
     values before moving them to the accumulator registers.  */
  if (mode == XOmode)
    {
      if (TARGET_MMA)
	{
	  ...
	  else if (VSX_REGNO_P (regno) && (regno & 1) == 0)
	    return 1;
	}
