			      ...
			      rtx tmp_altivec)
{
  ...
  if (MEM_P (src))
    {
      /* Right now, using prefixed addresses in the extract case generates
	 incorrect code.  Given an error until this can be fixed.  */
      if (address_is_prefixed (XEXP (src, 0), mode, INSN_FORM_DQ))
	error ("Using vector extracts on prefixed addresses is currently "
	       "broken");
...
rs6000_emit_move (rtx dest, rtx source, machine_mode mode)
{
  ...
  switch (mode)
	  ...
	  && small_data_operand (operands[1], mode))
	{
	  ...
	  && pcrel_local_or_external_address (operands[1], mode))
	{
	  emit_insn (gen_rtx_SET (operands[0], operands[1]));
	  return;
	}
...
address_to_insn_form (rtx addr, machine_mode mode, enum insn_form non_prefixed)
{
  ...
  if (non_prefixed == INSN_FORM_DEFAULT)
	...
	non_prefixed = INSN_FORM_DS;

      else if (TARGET_VSX && size >= 16
	       && ALTIVEC_OR_VSX_VECTOR_MODE (mode))
	non_prefixed = INSN_FORM_DQ;
...
/* Define ASM_OUTPUT_OPCODE to do anything special before emitting an opcode.
   We use it to emit a 'p' for prefixed insns that were scanned in
   FINAL_PRESCAN_INSN.  */
void
