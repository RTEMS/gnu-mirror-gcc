...
rs6000_maybe_emit_fp_cmove (rtx dest, rtx op, rtx true_cond, rtx false_cond)
{
  ...
  /* We allow the comparison to be either SFmode/DFmode and the true/false
    ...
    the 32/64-bit scalar types.  It gets too messy where SFmode/DFmode can use
    any register and TFmode/KFmode can only use Altivec registers.  In addtion,
    we would need to do a XXPERMDI if we compare SFmode/DFmode and move
    TFmode/KFmode.  */
