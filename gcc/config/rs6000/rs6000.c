      ...
      if (TARGET_64BIT)
	{
	  ...
	  if (TARGET_DIRECT_MOVE && !TARGET_DIRECT_MOVE_128)
	      ...
	      if (FLOAT128_VECTOR_P (TFmode))
		{
	      ...
	      if (TARGET_FUTURE)
		{
		  reg_addr[V2KFmode].reload_store = CODE_FOR_reload_v2kf_di_store;
		  reg_addr[V2KFmode].reload_load  = CODE_FOR_reload_v2kf_di_load;
		  reg_addr[V4KFmode].reload_store = CODE_FOR_reload_v4kf_di_store;
		  reg_addr[V4KFmode].reload_load  = CODE_FOR_reload_v4kf_di_load;
		}
	    }
	}
      else
...
rs6000_preferred_reload_class (rtx x, enum reg_class rclass)
{
  ...
  /* For the vector pair (V2KFmode) and vector quad (V4KFmode) modes, prefer
...
rs6000_split_multireg_move (rtx dst, rtx src)
{
  ...
  if (mode == V4KFmode && TARGET_VECTOR_256BIT && (MEM_P (dst) || MEM_P (src)))
    {
      reg_mode = V2KFmode;;
