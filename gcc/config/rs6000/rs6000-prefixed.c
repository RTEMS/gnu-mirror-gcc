...
prefixed_load_p (rtx_insn *insn)
{
  ...
  return prefixed_local_addr_p (addr, iform);
}
...
prefixed_store_p (rtx_insn *insn)
{
  ...
  enum insn_form iform = reg_to_insn_form (reg, GET_MODE (mem));

  return prefixed_local_addr_p (XEXP (mem, 0), iform);
}
...
prefixed_paddi_p (rtx_insn *insn)
{
  ...
  /* Look for either pc-relative addresses of local symbols that we can use a
     ...
     pc-relative load.  */
  return prefixed_local_addr_p (src, INSN_FORM_PREFIXED);
}
