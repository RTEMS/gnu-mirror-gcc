...
(define_mode_attr LARGE_GPR_LOAD [(DI "addis %0,%M1\;ld %0,%m1(%0)")
				  (SF "addis %0,%M1\;lwz %0,%m1(%0)")
				  (DF "addis %0,%M1\;ld %0,%m1(%0)")])

(define_mode_attr LARGE_FPR_LOAD [(DI "addis %2,%M1\;lfd %0,%m1(%2)")
				  (SF "addis %2,%M1\;lfs %0,%m1(%2)")
				  (DF "addis %2,%M1\;lfd %0,%m1(%2)")])

(define_mode_attr LARGE_VMX_LOAD [(DI "addis %2,%M1\;lxsd %0,%m1(%2)")
				  (SF "addis %2,%M1\;lxssp %0,%m1(%2)")
				  (DF "addis %2,%M1\;lxsd %0,%m1(%2)")])

;; Attribute for the store instruction for GPRs, FPRs, and Altivec registers.
(define_mode_attr LARGE_GPR_STORE [(DI "addis %2,%M0\;std %1,%m0(%2)")
				   (SF "addis %2,%M0\;stw %1,%m0(%2)")
				   (DF "addis %2,%M0\;std %1,%m0(%2)")])

(define_mode_attr LARGE_FPR_STORE [(DI "addis %2,%M0\;stfd %1,%m0(%2)")
				   (SF "addis %2,%M0\;stfs %1,%m0(%2)")
				   (DF "addis %2,%M0\;stfd %1,%m0(%2)")])

(define_mode_attr LARGE_VMX_STORE [(DI "addis %2,%M0\;stxsd %1,%m0(%2)")
				   (SF "addis %2,%M0\;stxssp %1,%m0(%2)")
				   (DF "addis %2,%M0\;stxsd %1,%m0(%2)")])

;; Insns for large addresses.  These are generated during the initial
;; expansion.  We typically don't split the insn until after register
;; allocation to cut down on the lifetime of the temporary.

(define_insn_and_split "large_mov<mode>_load"
  ...
  "@
   ...
   <LARGE_FPR_LOAD>
   <LARGE_VMX_LOAD>"
  "&& reload_completed && !TARGET_P9_FUSION
   && !base_reg_operand (operands[0], <MODE>mode)"
  [(set (match_dup 0)
	(match_dup 3))]
{
  rtx mem = operands[1];
  rtx new_addr = split_large_address (XEXP (mem, 0), operands[2]);
  operands[3] = change_address (mem, <MODE>mode, new_addr);
}
  [(set_attr "length" "8")
   (set_attr "type" "load,fpload,fpload")])

(define_insn_and_split "large_mov<mode>_store"
...

;; TOC breakout.  Normally we generate a separate TOC reference for each memory
;; reference.  Register allocation then splits this into ADDIS and the memory
;; operation.  This can result in a number of extra ADDIS's if there are
;; multiple references to different static variables or constant pool entries.
;;
...
(define_split
  [(set (match_operand:LARGE_INT 0 "toc_mem_operand")
	(match_operand:LARGE_INT 1 "gpc_reg_operand"))]
  "TARGET_TOC_BREAKOUT && TARGET_POWERPC64 && can_create_pseudo_p ()"
  [(const_int 0)]
{
  split_toc_memory (operands[0], operands[1]);
  DONE;
})
...
(define_split
  [(set (match_operand:LARGE_INT 0 "gpc_reg_operand")
	(match_operand:LARGE_INT 1 "toc_mem_operand"))]
  "TARGET_TOC_BREAKOUT && TARGET_POWERPC64 && can_create_pseudo_p ()"
  [(const_int 0)]
{
  split_toc_memory (operands[0], operands[1]);
...
;; Peephole2's to undo the previous TOC splitting if there is only one TOC
;; reference.
(define_peephole2
  [(set (match_operand:DI 0 "gpc_reg_operand")
	(high:DI (match_operand:DI 1 "small_toc_ref")))
   (set (match_dup 0)
	(lo_sum:DI (match_dup 0)
		   (match_dup 1)))
   (set (match_operand:LARGE_INT 2 "memory_operand")
	(match_operand:LARGE_INT 3 "gpc_reg_operand"))]
  "TARGET_TOC_BREAKOUT && TARGET_POWERPC64
   && rtx_referenced_p (operands[0], XEXP (operands[2], 0))
	...
	(match_dup 3))]
{
  ...
  if (REG_P (addr))
    ...
    addr2 = gen_rtx_PLUS (DImode, toc, XEXP (addr, 1));
  else
    gcc_unreachable ();

  new_addr = gen_rtx_LO_SUM (DImode, tmp, addr2);
  operands[4] = addr2;
...
(define_peephole2
  [(set (match_operand:DI 0 "gpc_reg_operand")
   ...
   (set (match_dup 0)
	(lo_sum:DI (match_dup 0)
		   (match_dup 1)))
   (set (match_operand:LARGE_INT 2 "gpc_reg_operand")
	(match_operand:LARGE_INT 3 "memory_operand"))]
  "TARGET_TOC_BREAKOUT && TARGET_POWERPC64
   && rtx_referenced_p (operands[0], XEXP (operands[3], 0))
   && peep2_reg_dead_p (3, operands[0])"
  [(set (match_dup 0)
	(high:DI (match_dup 4)))
   (set (match_dup 2)
