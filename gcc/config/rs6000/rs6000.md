...
; Iterator for integer types that need x-form addressing (reg+reg) for
; loads/stores <-> floating point/vector registers
(define_mode_iterator INT_XFORM [SI
				 (HI "TARGET_P9_VECTOR")
				 (QI "TARGET_P9_VECTOR")])

; Iterator for the floating point types for conversions to/from small integer
; types that need x-form addressing.
(define_mode_iterator FP_XFORM [SF
				DF
...
; used for DFmode instructions added in ISA 2.06 (power7), SFmode instructions
; added in ISA 2.07 (power8), and KFmode/TFmode instructions added in ISA 3.0
; (power9)
(define_mode_attr Fv2		[(SF "wy")
				 (DF "ws")
				 (DI "wi")
				 (KF "v")
				 (TF "v")])
...
;; Optimize fp convert and store to memory for small integer types that require
;; x-form (reg+reg) addressing.
(define_insn_and_split "fix_trunc<FP_XFORM:mode><INT_XFORM:mode>2_store"
  [(set (match_operand:INT_XFORM 0 "memory_operand" "=Z")
	(fix:INT_XFORM (match_operand:FP_XFORM 1 "gpc_reg_operand" "<Fv2>")))
   (clobber (match_scratch:INT_XFORM 2 "=<Fv2>"))]
  "TARGET_HARD_FLOAT && TARGET_P8_VECTOR"
  "#"
  "&& (reload_completed
       || (can_create_pseudo_p ()
	   && !indexed_or_indirect_operand (operands[0], SImode)))"
  [(set (match_dup 2)
	(fix:INT_XFORM (match_dup 1)))
   (set (match_dup 0)
	(match_dup 2))]
{
  rtx dest = operands[0];
  ...
  if (GET_CODE (operands[2]) == SCRATCH)
    operands[2] = gen_reg_rtx (<INT_XFORM:MODE>mode);

  if (can_create_pseudo_p ()
      && !indexed_or_indirect_operand (dest, <INT_XFORM:MODE>mode))
    operands[0] = rs6000_force_indexed_or_indirect_mem (dest);
}
  [(set_attr "length" "8")
   (set_attr "type" "fpstore")])

(define_insn_and_split "fixuns_trunc<FP_XFORM:mode><INT_XFORM:mode>2_store"
  [(set (match_operand:INT_XFORM 0 "memory_operand" "=Z")
	(unsigned_fix:INT_XFORM
	 (match_operand:FP_XFORM 1 "gpc_reg_operand" "<Fv2>")))
   (clobber (match_scratch:INT_XFORM 2 "=<Fv2>"))]
  "TARGET_HARD_FLOAT && TARGET_P8_VECTOR"
  "#"
  "&& (reload_completed
	   ...
	   && !indexed_or_indirect_operand (operands[0], SImode)))"
  [(set (match_dup 2)
	(unsigned_fix:INT_XFORM (match_dup 1)))
   (set (match_dup 0)
	(match_dup 2))]
{
  rtx dest = operands[0];
