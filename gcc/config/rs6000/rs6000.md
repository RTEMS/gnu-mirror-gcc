...
; the sign/zero extend insns are not defined, and the common code is used.
(define_insn "lfiwax"
...
;; Keep the SImode -> DImode convert along with DImode -> SF/DFmode through
...
;; The initial alternative is for power6, which supports LFIWAX but not VSX
;; constraints like "wa".
(define_insn_and_split "floatsi<mode>2_lfiwax"
  [(set (match_operand:SFDF 0 "gpc_reg_operand" "=d,wa,wa,wa,wa,wr")
	(float:SFDF
	...
	(float:SFDF (match_dup 2)))]
{
  ...
  operands[3] = (TARGET_POWERPC64
		 ? gen_extendsidi2 (tmp, src)
		 : gen_lfiwax (tmp, src));
}
  [(set_attr "length" "8,8,8,8,8,12")
   (set_attr "type" "fpload,fpload,fpload,mffgpr,fp,fp")
   (set_attr "isa" "*,p7v,p7v,p8v,p9v,p8v")])

;; LFIWZX LXSIWZX MTVSRWZ XXEXTRACTUW
;; LFIWZX was added in power7, so we don't need a non-VSX alternative.
(define_insn_and_split "floatunssi<mode>2_lfiwzx"
  [(set (match_operand:SFDF 0 "gpc_reg_operand" "=wa,wa,wa,wa")
	(unsigned_float:SFDF
	 (match_operand:SI 1 "nonimmediate_operand" "Z,Z,r,wa")))
   (clobber (match_scratch:DI 2 "=d,v,wa,wa"))]
  ...
  [(pc)]
{
  ...
  if (MEM_P (dest) && (TARGET_MFCRF || MEM_ALIGN (dest) >= 32))
  ...
  else
    {
      ...
      stack = rs6000_force_indexed_or_indirect_mem (stack);
  ...
  [(pc)]
{
  ...
  if (TARGET_POWERPC64)
    emit_insn (gen_extendsidi2 (tmp2, stack));
  else
  ...
  [(pc)]
{
  ...
  emit_insn (gen_stfiwx (stack, tmp1));
  if (TARGET_POWERPC64)
    emit_insn (gen_zero_extendsidi2 (tmp2, stack));
  else
