...
;; SFmode since we are moving it to a VSX register.  If the register being
;; loaded is a GPR, we need to load the SFmode value instead of the DFmode
;; value.
	...
	(unspec:SF [(match_dup 2)] UNSPEC_P8V_MTVSRD))]
{
  ...
  if (vsx_register_operand (dest, SFmode))
  ...
  else
    {
      ...
      if (SUBREG_P (dest))
	{
	  gcc_assert (SUBREG_BYTE (dest) == 0);
	  dest = SUBREG_REG (dest);
	}
      rtx reg_si = gen_rtx_SUBREG (SImode, dest, 0);
      emit_move_insn (reg_si, GEN_INT (l));
      DONE;
    }
}
  [(set_attr "type" "mftgpr,*")
