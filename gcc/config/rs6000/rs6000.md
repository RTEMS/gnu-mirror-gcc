...
;; direct move.  Note, we have transfer the constant as a double value instead
;; of single if the result is going to a VSX register, while if the desination
;; is a GPR we need to keep it as a single value.  The power8's direct move
;; was too slow, so only do this on power9 systems and later.
(define_insn_and_split "movsf_const_direct_move"
  [(set (match_operand:SF 0 "gpc_reg_operand" "=wa,b")
	...
	(match_dup 2))]
{
  ...
  /* VSX register, load up the double value and do a direct move.  */
  if (which_alternative == 0)
    {
      ...
      operands[4] = gen_rtx_REG (DImode, REGNO (dest));
    }

  /* GPR register, load up the single value.  */
  else
    {
      long l;
      REAL_VALUE_TO_TARGET_SINGLE (*CONST_DOUBLE_REAL_VALUE (src), l);
      emit_move_insn (gen_rtx_REG (DImode, REGNO (dest)), GEN_INT (l));
      DONE;
    }
})
...
(define_split
  [(set (match_operand:FMOVE64 0 "int_reg_operand")
	(match_operand:FMOVE64 1 "const_double_operand"))]
  "TARGET_POWERPC64 && reload_completed"
  [(set (match_dup 2) (match_dup 3))]
{
