...

;; Special case loading labels with toc relative addressing
;; Special case loading labels with toc relative addressing
(define_insn_and_split "*movdi_toc_label"
  [(set (match_operand:DI 0 "gpc_reg_operand" "=b")
  ...
  "&& 1"
  [(set (match_dup 0)
	(high:DI (unspec:DI [(match_dup 1)
			     (match_dup 2)] UNSPEC_TOC_LABEL)))
   (set (match_dup 0)
        (lo_sum:DI (match_dup 0)
		   (unspec:DI [(match_dup 1)
			       (match_dup 2)] UNSPEC_TOC_LABEL)))]
{
  operands[2] = gen_rtx_REG (DImode, TOC_REGNUM);
})

(define_insn "*movdi_toc_label_high"
  [(set (match_operand:DI 0 "gpc_reg_operand" "=b")
	(high:DI
...
(define_insn "*movdi_toc_label_low"
  [(set (match_operand:DI 0 "gpc_reg_operand" "+b")
	(lo_sum:DI
	 (match_dup 0)
	 (unspec:DI [(match_operand:DI 1 "label_ref_operand" "")
...
;; Use ADDPCIS to load up label constants.  The way ADDPCIS is used, is tied to
;; the 64-bit ELF assembler syntax.  Rs6000.c only enables -madd-pcrel on
;; 64-bit Linux systems.

(define_insn_and_split "movdi_add_pcrel"
  [(set (match_operand:DI 0 "gpc_reg_operand" "=b")
	(unspec:DI [(match_operand:DI 1 "add_pcrel_operand" "")]
		   UNSPEC_ADD_PCREL))]
  "TARGET_ADD_PCREL"
  "#"
  "&& 1"
  [(set (match_dup 0)
	(unspec:DI [(match_dup 1)
		    (match_dup 2)
		    (const_int 0)] UNSPEC_ADD_PCREL))
   (set (match_dup 0)
	(unspec:DI [(match_dup 1)
		    (match_dup 0)
		    (match_dup 2)
		    (const_int 1)] UNSPEC_ADD_PCREL))]
{
  /* Mark the particular ADDPCIS that pairs with the ADDI.  */
  static HOST_WIDE_INT add_pcrel_num = 0;
  operands[2] = GEN_INT (add_pcrel_num++);
})

;; Create .LADD_PCREL<n> to be the next instruction after the ADD_PCRELIS, so that
;; the ADDI later can use the label to generate the bottom 16-bits.
(define_insn "*movdi_add_pcrel_internal1"
  [(set (match_operand:DI 0 "gpc_reg_operand" "=b")
	(unspec:DI [(match_operand:DI 1 "add_pcrel_operand" "")
		    (match_operand:DI 2 "const_int_operand" "n")
		    (const_int 0)] UNSPEC_ADD_PCREL))]
  "TARGET_ADD_PCREL"
  "addpcis %0,(%1-.LADD_PCREL%2)@ha\n.LADD_PCREL%2:"
  [(set_attr "type" "mtjmpr")
   (set_attr "cost" "6")])

(define_insn "*movdi_add_pcrel_internal2"
  [(set (match_operand:DI 0 "gpc_reg_operand" "+b")
	(unspec:DI [(match_operand:DI 1 "add_pcrel_operand" "")
		    (match_dup 0)
		    (match_operand:DI 2 "const_int_operand" "n")
