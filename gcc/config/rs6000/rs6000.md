...
;; On 64-bit systems that allow SImode in vector registers, just convert to DImode.
;; On earlier systems and 32-bit systems, generate LFIWAX/LFIWZX instructions
;; directly.
(define_expand "floatsi<mode>2_lfiwax"
  [(set (match_dup 2)
   ...
   && <SI_CONVERT_FP> && can_create_pseudo_p ()"
{
  ...
  if (MEM_P (src))
    operands[1] = src = rs6000_force_indexed_or_indirect_mem (src);
  ...
  if (TARGET_DIRECT_MOVE_64BIT)
    ...
    {
      if (!MEM_P (src) && !TARGET_DIRECT_MOVE)
	{
	  rtx stack_tmp = rs6000_allocate_stack_temp (SImode, false, true);
	  emit_move_insn (stack_tmp, src);
	  src = stack_tmp;
	}
      emit_insn (gen_lfiwax (di_tmp, src));
      emit_insn (gen_floatdi<mode>2 (dest, di_tmp));
      DONE;
    }
})

(define_expand "floatunssi<mode>2_lfiwzx"
  [(set (match_dup 2)
   ...
   && can_create_pseudo_p ()"
{
  ...
  if (TARGET_DIRECT_MOVE_64BIT)
      ...
      if (!MEM_P (src) && !TARGET_DIRECT_MOVE)
	{
      ...
      emit_insn (gen_lfiwzx (di_tmp, src));
      emit_insn (gen_floatdi<mode>2 (dest, di_tmp));
      DONE;
    }
})
