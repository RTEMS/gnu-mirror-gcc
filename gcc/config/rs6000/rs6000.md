...
(define_attr "isa" "any,p5,p6,p7,p7v,p8v,p9,p9v,p9kf,p9tf,p10,quad"
  (const_string "any"))
...
(define_attr "enabled" ""
  (cond
    [(eq_attr "isa" "any")
     ...
     (and (eq_attr "isa" "quad")
	  (match_test "TARGET_QUAD_ARITHMETIC"))
     (const_int 1)
    ] (const_int 0)))
...
;; TImode set condition mask
(define_insn_and_split "*mov<mode>ti_setmask"
  [(set (match_operand:FPMASK 0 "vsx_register_operand" "=<Fv>")
	(if_then_else:FPMASK
	 (match_operator:CC 1 "vcmp_ti_split_operator"
	    [(match_operand:TI 2 "altivec_register_operand" "v")
	     (match_operand:TI 3 "altivec_register_operand" "v")])
	 (match_operand:FPMASK 4 "vsx_register_operand" "<Fv>")
	 (match_operand:FPMASK 5 "vsx_register_operand" "<Fv>")))
   (clobber (match_scratch:V2DI 6 "=&v"))]
  "TARGET_P9_MINMAX"
  "#"
  "&& 1"
  [(pc)]
{
  ...
  /* See if we need to reverse the comparsion and true/false conditions.  */
  if (!vcmp_ti_strict_operator (cmp, CCmode))
    {
      enum rtx_code code = reverse_condition (GET_CODE (cmp));
      std::swap (cmp_op1, cmp_op2);
      cmp = gen_rtx_fmt_ee (code, CCmode, cmp_op1, cmp_op2);
    }

  /* Emit the compare and set mask instruction.  */
  emit_insn (gen_setmaskti (mask_reg, cmp, cmp_op1, cmp_op2, mask_m1, mask_0));

  /* Now emit the XXSEL insn.  */
  emit_insn (gen_xxsel<mode> (dest, mask_reg, mask_0, move_t, move_f));
  DONE;
}
 [(set_attr "length" "8")
  (set_attr "type" "vecperm")])

(define_insn "setmaskti"
  [(set (match_operand:V2DI 0 "altivec_register_operand" "=v")
	(if_then_else:V2DI
	 (match_operator:CC 1 "vcmp_ti_strict_operator"
		[(match_operand:TI 2 "altivec_register_operand" "v")
		 (match_operand:TI 3 "altivec_register_operand" "v")])
	 (match_operand:V2DI 4 "all_ones_constant" "")
	 (match_operand:V2DI 5 "zero_constant" "")))]
  "TARGET_TI_COMPARE_MASK"
{
  switch (GET_CODE (operands[1]))
  {
    case EQ:  return "vcmpequq %0,%2,%3";
    case GT:  return "vcmpgtsq %0,%2,%3";
    case GTU: return "vcmpgtuq %0,%2,%3";
    case LT:  return "vcmpgtsq %0,%3,%2";
    case LTU: return "vcmpgtuq %0,%3,%2";
    default:  break;
  }
...
(define_expand "addti3"
  ...
  "TARGET_64BIT"
{
  /* Do we want to use the vector unit only for TImode arithmetic?  */
  if (TARGET_QUAD_ARITHMETIC)
    {
      if (CONST_INT_P (operands[2]))
	operands[2] = force_reg (TImode, operands[2]);

      emit_insn (gen_addti3_vadduqm (operands[0], operands[1], operands[2]));
      DONE;
    }
...
;; Implement quad add using Altivec registers.
(define_insn "addti3_vadduqm"
  [(set (match_operand:TI 0 "gpc_reg_operand" "=v")
	(plus:TI (match_operand:TI 1 "gpc_reg_operand" "v")
		 (match_operand:TI 2 "gpc_reg_operand" "v")))]
  "TARGET_QUAD_ARITHMETIC"
  "vadduqm %0,%1,%2"
  [(set_attr "type" "vecsimple")])

;; If -mquad-add-subtract, delay splitting up the subtract until after
;; register allocation to allow using vadduqm.
  ...
  "TARGET_64BIT"
{
  ...
  if (TARGET_QUAD_ARITHMETIC)
	...
	operands[1] = force_reg (TImode, operands[1]);
      ...
      emit_insn (gen_subti3_vsubuqm (operands[0], operands[1], operands[2]));
      DONE;
    }
...
;; Implement quad subtract for Altivec registers.
(define_insn "subti3_vsubuqm"
  [(set (match_operand:TI 0 "gpc_reg_operand" "=v")
...
(define_expand "negti2"
  [(set (match_operand:TI 0 "gpc_reg_operand")
	(neg:TI (match_operand:TI 1 "gpc_reg_operand")))]
  "TARGET_QUAD_ARITHMETIC"
{
  rtx vzero = gen_reg_rtx (TImode);
  emit_move_insn (vzero, const0_rtx);
  emit_insn (gen_subti3_vsubuqm (operands[0], vzero, operands[1]));
  DONE;
})
