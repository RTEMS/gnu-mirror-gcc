...
;; Float128 conversion functions.  These expand to library function calls if
;; the types don't use the same representation, and dummy conversion insns if
;; they do use the same representation.  We use FLOAT_EXTEND to convert from
;; IFmode/KFmode to TFmode and FLOAT_TRUNCATE to convert from TFmode to
;; IFmode/KFmode, because TFmode is numerically higher than IFmode/KFmode, due
;; to the ordering in rs6000-modes.def.
(define_expand "extend<mode>tf2"
  [(set (match_operand:TF 0 "gpc_reg_operand")
	(float_extend:TF (match_operand:IFKF 1 "gpc_reg_operand")))]
  "TARGET_FLOAT128_TYPE"
{
  if (FLOAT128_IBM_P (TFmode) != FLOAT128_IBM_P (<MODE>mode))
    {
      rs6000_expand_float128_convert (operands[0], operands[1], false);
      DONE;
    }
