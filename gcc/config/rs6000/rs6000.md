   ...
   (match_dup 13)]
{
  ...
  operands[15] = force_const_mem (DFmode,
				  const_double_from_real_value (dconst0,
								DFmode));
  if (TARGET_PCREL || TARGET_TOC)
    {
      rtx ref14, ref15;
      rtx sym14 = XEXP (operands[14], 0);
      rtx sym15 = XEXP (operands[15], 0);
      alias_set_type alias = get_data_alias_set ();

      if (TARGET_PCREL)
	{
	  ref14 = create_data_reference (sym14);
	  ref15 = create_data_reference (sym15);
	}
      else
	{
	  rtx tmp_reg = operands[11];
	  ref14 = create_TOC_reference (sym14, tmp_reg);
	  ref15 = create_TOC_reference (sym15, tmp_reg);
	}

      operands[14] = gen_const_mem (DFmode, ref14);
      operands[15] = gen_const_mem (DFmode, ref15);
      set_mem_alias_set (operands[14], alias);
      set_mem_alias_set (operands[15], alias);
    }
})

