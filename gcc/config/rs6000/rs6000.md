...
(define_code_attr     SMINMAX	[(smin "SMIN")
				 (smax "SMAX")])
;; Iterator for using the minimum or maximum instruction.  With ISA 3.0
;; (power9), we use the "C" min/max instructions.  With earlier machines, we
;; used the original min/max instructions if -ffast-math was used.  Likewise
;; for ISA 3.1 (power10), we now support "C" min/max for IEEE 128-bit floating
;; point.
(define_mode_iterator FMIN [(SF "TARGET_VSX && TARGET_HARD_FLOAT")
			    (DF "TARGET_VSX && TARGET_HARD_FLOAT")
			    (KF "have_compare_and_set_mask (KFmode)")
			    (TF "have_compare_and_set_mask (TFmode)")])

;; Iterator to allow doing floating point scalar conditional moves.  Having two
;; iterators (FMIN, FCMOVE) allows us to do a conditional move of a floating
;; point type where the test for doing the condition move is another floating
;; point type.
(define_mode_iterator FCMOVE [(SF "have_compare_and_set_mask (SFmode)")
			      (DF "have_compare_and_set_mask (DFmode)")
			      (KF "have_compare_and_set_mask (KFmode)")
			      (TF "have_compare_and_set_mask (TFmode)")])

;; Mode attribute to give the constraint to use for the minimum, maximum, and
;; set compare mask instructions.
(define_mode_attr Fm	[(SF "wa") (DF "wa") (KF "v") (TF "v")])
...
;;
;; On ISA 3.1 (power10), we expand the support to include IEEE 128-bit floating
;; point.

(define_expand "s<minmax><mode>3"
  [(set (match_operand:FMIN 0 "gpc_reg_operand")
	(fp_minmax:FMIN
	 (match_operand:FMIN 1 "gpc_reg_operand")
...
(define_expand "mov<mode>cc"
   [(set (match_operand:FMIN 0 "gpc_reg_operand")
	 (if_then_else:FMIN (match_operand 1 "comparison_operator")
			    (match_operand:FMIN 2 "gpc_reg_operand")
			    (match_operand:FMIN 3 "gpc_reg_operand")))]
...
;; Handle conditional move if we have a compare and set mask instruction
(define_insn_and_split "*mov<FMIN:mode><FCMOVE:mode>cc"
  [(set (match_operand:FMIN 0 "vsx_register_operand" "=<FMIN:Fm>")
	(if_then_else:FMIN
	 (match_operator:CCFP 1 "fpmask_comparison_operator"
		[(match_operand:FCMOVE 2 "vsx_register_operand" "<FCMOVE:Fm>")
		 (match_operand:FCMOVE 3 "vsx_register_operand" "<FCMOVE:Fm>")])
	 (match_operand:FMIN 4 "vsx_register_operand" "<FMIN:Fm>")
	 (match_operand:FMIN 5 "vsx_register_operand" "<FMIN:Fm>")))
   (clobber (match_scratch:V2DI 6 "=&<FCMOVE:Fm>"))]
  "TARGET_P9_MINMAX"
  "#"
  "&& 1"
  [(set (match_dup 6)
	(if_then_else:V2DI (match_dup 1)
			   ...
			   (match_dup 8)))
   (set (match_dup 0)
	(if_then_else:FMIN (ne (match_dup 6)
			       (match_dup 8))
			   (match_dup 4)
			   (match_dup 5)))]
{
...
(define_insn_and_split "*mov<FMIN:mode><FCMOVE:mode>cc_invert"
  [(set (match_operand:FMIN 0 "vsx_register_operand" "=<FMIN:Fm>")
	(if_then_else:FMIN
...
(define_insn "*fpmask<mode>"
  [(set (match_operand:V2DI 0 "vsx_register_operand" "=<Fm>")
	(if_then_else:V2DI
	 (match_operator:CCFP 1 "fpmask_comparison_operator"
		[(match_operand:FMIN 2 "vsx_register_operand" "<Fm>")
		 (match_operand:FMIN 3 "vsx_register_operand" "<Fm>")])
...
(define_insn "*xxsel<mode>"
  [(set (match_operand:FMIN 0 "vsx_register_operand" "=<Fm>")
	(if_then_else:FMIN (ne (match_operand:V2DI 1 "vsx_register_operand" "wa")
			       (match_operand:V2DI 2 "zero_constant" ""))
			   (match_operand:FMIN 3 "vsx_register_operand" "<Fm>")
			   (match_operand:FMIN 4 "vsx_register_operand" "<Fm>")))]
  "TARGET_P9_MINMAX"
  "xxsel %x0,%x4,%x3,%x1"
