...
(define_mode_attr LADDR_GLOAD	[(QI "addis %0,%M1\;lbz %0,%m1(%0)")
				 (HI "addis %0,%M1\;lhz %0,%m1(%0)")
				 (SI "addis %0,%M1\;lwz %0,%m1(%0)")
				 (DI "addis %0,%M1\;ld %0,%m1(%0)")
				 (SF "addis %0,%M1\;lwz %0,%m1(%0)")
				 (DF "addis %0,%M1\;ld %0,%m1(%0)")])

(define_mode_attr LADDR_FLOAD	[(QI "error")
				 (HI "error")
				 (SI "error")
				 (DI "addis %2,%M1\;lfd %0,%m1(%2)")
				 (SF "addis %2,%M1\;lfs %0,%m1(%2)")
				 (DF "addis %2,%M1\;lfd %0,%m1(%2)")])

(define_mode_attr LADDR_VLOAD	[(QI "error")
				 (HI "error")
				 (SI "error")
				 (DI "addis %2,%M1\;lxsd %0,%m1(%2)")
...
(define_mode_attr LADDR_FSTORE [(QI "error")
				(HI "error")
				(SI "error")
				(DI "addis %2,%M0\;stfd %1,%m0(%2)")
				(SF "addis %2,%M0\;stfs %1,%m0(%2)")
				(DF "addis %2,%M0\;stfd %1,%m0(%2)")])

(define_mode_attr LADDR_VSTORE [(QI "error")
				(HI "error")
				(SI "error")
				(DI "addis %2,%M0\;stxsd %1,%m0(%2)")
				(SF "addis %2,%M0\;stxssp %1,%m0(%2)")
				(DF "addis %2,%M0\;stxsd %1,%m0(%2)")])

;; Insns for large addresses.  The load insn is not generated by default for
;; integers, instead we use power8 fusion and use the register as a base
;; register.  INT->FP conversions do use the load.  The store insn is generated
;; and split after register allocation to cut down on the lifetime of the
;; temporary.

(define_insn "large_mov<mode>_load"
  [(set (match_operand:QHSI 0 "gpc_reg_operand" "=b,b")
	(unspec:QHSI [(match_operand:QHSI 1 "large_mem_operand" "wC,wC")]
		     UNSPEC_LADDR_LOAD))
   (clobber (match_operand:DI 2 "base_reg_operand" "=0,&b"))]
...
(define_insn "large_mov<mode>_store"
  [(set (match_operand:QHSI 0 "large_mem_operand" "=wC")
	(unspec:QHSI [(match_operand:QHSI 1 "gpc_reg_operand"  "r")]
		     UNSPEC_LADDR_STORE))
   (clobber (match_operand:DI 2 "base_reg_operand" "=&b"))]
  "TARGET_LARGE_ADDRESS && TARGET_POWERPC64"
  "<LADDR_GSTORE>"
  [(set_attr "length" "8")
   (set_attr "type" "store")])
...
(define_insn "large_movdi_load"
  [(set (match_operand:DI 0 "gpc_reg_operand"               "=b,b, d,wb")
	(unspec:DI [(match_operand:DI 1 "large_mem_operand" "wC,wC,wC,wC")]
		   UNSPEC_LADDR_LOAD))
   (clobber (match_operand:DI 2 "base_reg_operand"          "=0,&b,b, b"))]
  "TARGET_LARGE_ADDRESS && TARGET_POWERPC64"
  "@
   addis %0,%M1\;ld %0,%m1(%0)
   addis %2,%M1\;ld %0,%m1(%2)
   addis %2,%M1\;lfd %0,%m1(%2)
   addis %2,%M1\;lxsd %0,%m1(%2)"
  [(set_attr "length" "8")
   (set_attr "type" "load,load,fpload,fpload")])

(define_insn "large_movdi_store"
  [(set (match_operand:DI 0 "large_mem_operand"           "=wC,wC,wC")
	(unspec:DI [(match_operand:DI 1 "gpc_reg_operand"  "r, d, wb")]
		   UNSPEC_LADDR_STORE))
   (clobber (match_operand:DI 2 "base_reg_operand"        "=&b,b, b"))]
  "TARGET_LARGE_ADDRESS && TARGET_POWERPC64"
  "@
   addis %2,%M0\;std %1,%m0(%2)
   addis %2,%M0\;stfd %1,%m0(%2)
   addis %2,%M0\;stxsd %1,%m0(%2)"
  [(set_attr "length" "8")
   (set_attr "type" "store,fpstore,fpstore")])

(define_insn "large_mov<mode>_load"
  [(set (match_operand:SFDF 0 "gpc_reg_operand" "=d,wb")
	(unspec:SFDF [(match_operand:SFDF 1 "large_mem_operand" "wC,wC")]
		     UNSPEC_LADDR_LOAD))
...
(define_insn "large_mov<mode>_store"
  [(set (match_operand:SFDF 0 "large_mem_operand" "=wC,wC")
	(unspec:SFDF [(match_operand:SFDF 1 "gpc_reg_operand"  "d,wb")]
		     UNSPEC_LADDR_STORE))
   (clobber (match_operand:DI 2 "base_reg_operand" "=b,b"))]
  "TARGET_LARGE_ADDRESS && TARGET_POWERPC64"
  "@
...
(define_insn_and_split "*float<uns>di<mode>2_mem"
  [(set (match_operand:SFDF 0 "gpc_reg_operand"  "=d,wa,wa,wb,wa,wb")
	(any_float:SFDF
	 (match_operand:DI 1 "any_mem_operand"   "wN,wN,Z, wY,wC,wC")))
   (clobber (match_scratch:DI 2                  "=d,d, wa,wa,d, wa"))
   (clobber (match_scratch:DI 3                  "=X,X, X, X, &b,&b"))]
  "TARGET_<MODE>_FPR && <FP_CVT_P> && <SI_CONVERT_FP>"
  "#"
  "&& reload_completed"
   ...
   (set (match_dup 0) (any_float:SFDF (match_dup 2)))]
{
  ...
  if (large_address_valid (addr, DImode))
      ...
      if (TARGET_P9_FUSION)
	{
	  ...
	  emit_insn (gen_large_movdi_load (fpr_tmp, mem, base_reg));
