...
;; If -mquad-add-subtract, delay splitting up the add until after
  ...
  "TARGET_64BIT"
{
  if (TARGET_QUAD_ARITHMETIC)
    {
      /* Make sure any constants do not need to be loaded with force_reg if we
         are possibly generating vadduqm.  */
      if (CONST_INT_P (operands[2]))
	{
	  rtx lo2 = gen_lowpart (DImode, operands[2]);
	  rtx hi2 = gen_highpart_mode (DImode, TImode, operands[2]);
	  if (!short_cint_operand (lo2, DImode) || !adde_operand (hi2, DImode))
	    operands[2] = force_reg (TImode, operands[2]);
	}

      emit_insn (gen_addti3_quad (operands[0], operands[1], operands[2]));
      DONE;
    }
...
;; Implement quad add for both GPRs and Altivec registers.  Prefer the GPRs,
;; but provide the code in case the TImode happens to be in Altivec registers.
(define_insn_and_split "addti3_quad"
  [(set (match_operand:TI 0 "gpc_reg_operand"              "=r, v")
	(plus:TI (match_operand:TI 1 "gpc_reg_operand"      "r, v")
		 (match_operand:TI 2 "reg_or_short_operand" "ri,v")))]
  ...
  "TARGET_64BIT"
{
  if (TARGET_QUAD_ARITHMETIC)
      ...
      if (CONST_INT_P (operands[2]))
	{
	  ...
	  if (!short_cint_operand (lo1, DImode) || !adde_operand (hi1, DImode))
	    operands[1] = force_reg (TImode, operands[1]);
	}

      emit_insn (gen_subti3_quad (operands[0], operands[1], operands[2]));
      DONE;
    }
...
;; Implement quad subtract for both GPRs and Altivec registers.  Prefer the GPRs,
;; but provide the code in case the TImode happens to be in Altivec registers.
(define_insn_and_split "subti3_quad"
  [(set (match_operand:TI 0 "gpc_reg_operand"               "=r, v")
	(minus:TI (match_operand:TI 1 "reg_or_short_operand" "ri,v")
		  (match_operand:TI 2 "gpc_reg_operand"      "r, v")))]
  "TARGET_QUAD_ARITHMETIC"
  "@
   #
   vsubqum %0,%1,%2"
