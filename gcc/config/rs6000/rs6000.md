...
(define_insn "large_mov<mode>_load"
  [(set (match_operand:QHSI 0 "base_reg_operand" "=b")
	(unspec:QHSI [(match_operand:QHSI 1 "large_mem_operand" "wC")]
		     UNSPEC_LADDR_LOAD))]
...
(define_insn "large_mov<mode>_store"
  [(set (match_operand:QHSI 0 "large_mem_operand" "=wC")
	(unspec:QHSI [(match_operand:QHSI 1 "int_reg_operand"  "r")]
		     UNSPEC_LADDR_STORE))
...
(define_insn "large_movdi_load"
  [(set (match_operand:DI 0 "gpc_reg_operand" "=b,*d,*wb")
	(unspec:DI [(match_operand:DI 1 "large_mem_operand" "wC,wC,wC")
		    (const_int 0)]
		   UNSPEC_LADDR_LOAD))
...
(define_insn "large_movdi_load_fp"
  [(set (match_operand:DI 0 "vsx_register_operand" "=d,wb")
	(unspec:DI [(match_operand:DI 1 "large_mem_operand" "wC,wC")
		    (const_int 1)]
		   UNSPEC_LADDR_LOAD))
  ...
  "TARGET_LARGE_ADDRESS && TARGET_POWERPC64"
{
  ...
  rtx mem = operands[1];
  rtx tmp = GET_CODE (operands[2]) == SCRATCH ? reg : operands[2];
  static const char *const fload[3] = { "lfd", "lxsd" };
  output_large_address_load_store (reg, mem, tmp, fload[which_alternative]);
  return "";
}
  [(set_attr "length" "8")
   (set_attr "type" "load")])
  ...
  "TARGET_LARGE_ADDRESS && TARGET_POWERPC64"
{
  ...
  rtx reg = operands[1];
  rtx tmp = operands[2];
  static const char *const store[3] = { "std", "stfd", "stxsd" };
...
(define_insn "large_mov<mode>_load"
  [(set (match_operand:SFDF 0 "gpc_reg_operand" "=d,wb,*b")
	(unspec:SFDF [(match_operand:SFDF 1 "large_mem_operand" "wC,wC,wC")]
		     UNSPEC_LADDR_LOAD))
...
(define_insn "large_mov<mode>_store"
  [(set (match_operand:SFDF 0 "large_mem_operand" "=wC,wC,wC")
	(unspec:SFDF [(match_operand:SFDF 1 "gpc_reg_operand"  "d,wb,*r")]
		     UNSPEC_LADDR_STORE))
   (clobber (match_scratch:DI 2 "=b,b,&b"))]
  "TARGET_LARGE_ADDRESS && TARGET_POWERPC64"
{
  ...
  [(const_int 0)]
{
  ...
  if (TARGET_P9_FUSION
      || (TARGET_P8_FUSION && GET_MODE_CLASS (<MODE>mode) == MODE_INT))
    {
  ...
  rtx new_addr = split_large_address (XEXP (src, 0), tmp);
  rtx new_mem = replace_equiv_address (src, new_addr);
  ...
  [(const_int 0)]
{
  ...
  rtx tmp = gen_reg_rtx (DImode);
  rtx new_addr = split_large_address (XEXP (dest, 0), tmp);
  rtx new_mem = replace_equiv_address (dest, new_addr);
...
(define_insn_and_split "ashdi3_extswsli"
  [(set (match_operand:DI 0 "gpc_reg_operand" "=r,r")
	(ashift:DI
	 (sign_extend:DI (match_operand:SI 1 "reg_or_mem_operand" "r,wN"))
	 (match_operand:DI 2 "u6bit_cint_operand" "n,n")))]
  "TARGET_EXTSWSLI && !large_mem_operand (operands[1], SImode)"
  "@
   extswsli %0,%1,%2
   #"
...
(define_insn_and_split "unpack<mode>_nodm"
  [(set (match_operand:<FP128_64> 0 "nonimmediate_operand" "=d,wN")
	(unspec:<FP128_64>
