...
; Different instruction formats that are used by GCC in determining whether an
; address is a simple offsettable address or whether it is a prefixed address.
; There are various other instruction formats used by the PowerPC, but the
; these formats are not currently used.
(define_enum "insn_form" [bad			; bad instruction form
			  default		; use default format
			  base_reg		; base register only
			  d			; base + 16-bit offset
			  ds			; base + 14-bit offset
			  dq			; base + 12-bit offset
			  x			; base + index
			  update		; address udpates base reg.
			  lo_sum		; special offset insn.
			  prefixed_numeric	; base + 34 bit offset
			  pcrel_local		; pc-relative local sym.
			  pcrel_external])	; pc-relative external sym.
...
;; to the symbol or label.  Even though the pc-relative load address
;; instruction is 8 bytes, we set the length to 12 since the assembler and
;; linker need to align these instructions to a 64 byte boundary.
