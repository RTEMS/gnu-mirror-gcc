...
;; Use ADDPCIS to load up label constants.  This must appear before movdi so
;; that this pattern matches before the general one.  The way ADDPCIS is used,
;; it is tied to the 64-bit ELF assembler syntax.

(define_insn_and_split "*movdi_addpcis"
  [(set (match_operand:DI 0 "gpc_reg_operand" "=b")
	(match_operand:DI 1 "addpcis_label_ref_operand" ""))]
  "TARGET_ADDPCIS"
  "#"
  "&& 1"
  [(set (match_dup 0)
	(unspec:DI [(match_dup 1)
		    (const_int 0)] UNSPEC_ADDPCIS))
   (set (match_dup 0)
	(unspec:DI [(match_dup 1)
		    (match_dup 0)
		    (const_int 1)] UNSPEC_ADDPCIS))])

;; Create .Laddpcis<n> to be the difference between the next instruction and
;; the label we are loading the address for so that the ADDI later can use the
;; label to generate the bottom 16-bits.
(define_insn "*movdi_addpcis_internal1"
  [(set (match_operand:DI 0 "gpc_reg_operand" "=b")
	(unspec:DI [(match_operand:DI 1 "addpcis_label_ref_operand" "")
		    (const_int 0)] UNSPEC_ADDPCIS))]
  "TARGET_ADDPCIS"
{
  operands[2] = GEN_INT (CODE_LABEL_NUMBER (label_ref_label (operands[1])));
  return "addpcis %0,.Laddpcis%2@ha\n.Laddpcis%2 = %1-.";
}
  [(set_attr "type" "mtjmpr")
   (set_attr "cost" "6")])

(define_insn "*movdi_addpcis_internal2"
  [(set (match_operand:DI 0 "gpc_reg_operand" "+b")
	(unspec:DI [(match_operand:DI 1 "addpcis_label_ref_operand" "")
		    (match_dup 0)
		    (const_int 1)] UNSPEC_ADDPCIS))]
