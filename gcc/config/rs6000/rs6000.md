...
;; If we are zero-extending (SUBREG:SI (REG:SF ...)), we need to convert the
;; SFmode from internal format to storage format if we have direct move where
;; we can avoid using store to create the value.
(define_expand "zero_extendsi<mode>2"
  ...
  ""
{
  if (TARGET_DIRECT_MOVE_64BIT && !reload_in_progress && !reload_completed
      && !lra_in_progress && SUBREG_P (operands[1])
      && GET_MODE (SUBREG_REG (operands[1])) == SFmode)
    {
      rtx tmp = gen_reg_rtx (SImode);
      rtx inner_op1 = SUBREG_REG (operands[1]);
      machine_mode inner_mode = GET_MODE (inner_op1);

      if (inner_mode == SFmode)
	{
	  emit_insn (gen_movsi_from_sf (tmp, inner_op1));
	  operands[1] = tmp;
	}
    }
})
...
(define_insn "*zero_extendsi<mode>2_internal"
...
;; If we are sign-extending (SUBREG:SI (REG:SF ...)), we need to convert the
;; SFmode from internal format to storage format if we have direct move where
;; we can avoid using store to create the value.
(define_expand "extendsi<mode>2"
  [(set (match_operand:EXTSI 0 "gpc_reg_operand")
	(sign_extend:EXTSI (match_operand:SI 1 "reg_or_mem_operand")))]
  ""
{
  if (TARGET_DIRECT_MOVE_64BIT && !reload_in_progress && !reload_completed
      && !lra_in_progress && SUBREG_P (operands[1])
...
(define_insn "*extendsi<mode>2_internal"
...
(define_insn_and_split "movsf_from_si"
  [(set (match_operand:SF 0 "register_operand"
	...
	(unspec:SF [(match_operand:SI 1 "register_operand" 
	    "m,         m,         wY,        Z,         r,         f,
	     wu,        r,         wy,        r")]
		   UNSPEC_SF_FROM_SI))
  ...
  [(const_int 0)]
{
  ...
  rtx op1 = operands[1];
  rtx op2 = operands[2];
  rtx op1_di = gen_rtx_REG (DImode, REGNO (operands[1]));
