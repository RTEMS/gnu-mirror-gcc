...
;; the 64-bit ELF assembler syntax.  Rs6000.c only enables -maddpcis on 64-bit
;; Linux systems.
		    ...
		    (const_int 1)] UNSPEC_ADDPCIS))]
{
  ...
  static HOST_WIDE_INT addpcis_num = 0;
  operands[2] = GEN_INT (addpcis_num++);
})

;; Create .LADDPC<n> to be the next instruction after the ADDPCIS, so that
