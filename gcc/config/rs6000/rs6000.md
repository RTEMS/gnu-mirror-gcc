...
(define_split
  [(set (match_operand:FMOVE32 0 "int_reg_operand")
...
(define_insn "movsf_hardfloat"
  ...
  "(register_operand (operands[0], SFmode)
   || register_operand (operands[1], SFmode))
   && TARGET_HARD_FLOAT
   && !easy_fp_direct_move_constant (operands[1], SFmode)
...
;; direct move.  Note, we have transfer the constant as a DFmode value instead
;; of SFmode if the result is going to a VSX register, while if the desination
;; is a GPR we need to keep it as a SFmode.  The power8's direct move is too
;; slow, so only do this on power9 systems and later.
