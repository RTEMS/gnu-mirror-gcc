...
;;			MR       LWZ      LFIWZX   LXSIWZX  STW      STFS
;;			STXSSP   STXSSPX  VSX->GPR MTVSRWZ  VSX->VSX
...
(define_insn_and_split "movsi_from_sf"
  ...
  "@
   ...
   #"
  "&& reload_completed
   && ((int_reg_operand (operands[0], SImode) && TARGET_DIRECT_MOVE_64BIT)
       || (altivec_register_operand (operands[0], SImode) && TARGET_P9_VECTOR))
  ...
  [(const_int 0)]
{
  ...
  [(set_attr "type"	"*,      load,    fpload,  fpload,  store,   fpstore,
			fpstore, fpstore, mftgpr,  mffgpr,  vecperm")
   (set_attr "length"   "4,      4,       4,       4,       4,       4,
			 4,      4,       12,      4,       8")])

;; movsi_from_sf with zero extension
;;			RLDICL  LWZ   LFIWZX  LXSIWZX VSX->GPR MTVSRWZ VSX->VSX
  ...
  [(const_int 0)]
{
  ...
  if (int_reg_operand (op0, DImode))
  ...
  else
    {
      rtx op1_v16qi = gen_rtx_REG (V16QImode, REGNO (op1));
      rtx byte_off = VECTOR_ELT_ORDER_BIG ? const0_rtx : GEN_INT (12);
      emit_insn (gen_vextract4b (op0, op1_v16qi, byte_off));
    }

  DONE;
}
  [(set_attr "type"	"*,     load, fpload, fpload, mftgpr,  mffgpr, vecperm")
   (set_attr "length"	"4,     4,    4,      4,      12,      4,      8")])

;; movsi_from_sf with sign extension
;;
;;			EXTSW    LWA   LFIWAX  LXSIWAX VSX->GPR MTVSRWA

(define_insn_and_split "*movdi_from_sf_sign_ext"
  [(set (match_operand:DI 0 "gpc_reg_operand"
			"=r,     r,    ?*wI,   ?*wH,   r,       wIwH")
  ...
  "@
   ...
   #
   mtvsrwa %x0,%1"
  "&& reload_completed && TARGET_DIRECT_MOVE_64BIT
  ...
  [(const_int 0)]
{
  ...
  emit_insn (gen_vsx_xscvdpspn_scalar (op2, op1));
  emit_insn (gen_p8_mfvsrd_4_disf (op0, op2));
  emit_insn (gen_ashrdi3 (op0, op0, GEN_INT (32)));
  DONE;
}
  [(set_attr "type"	"*,      load, fpload, fpload, mftgpr,  mffgpr")
   (set_attr "length"	"4,      4,    4,      4,      12,      4")])
...
(define_insn_and_split "movsf_from_si"
  [(set (match_operand:SF 0 "gpc_reg_operand"
	...
	(unspec:SF [(match_operand:SI 1 "gpc_reg_operand" 
	    "m,         m,         wY,        Z,         r,         f,
	     wu,        wr,        wy,        r")]
		   UNSPEC_SF_FROM_SI))
