...
(define_expand "fix<uns>_trunc<SFDF:mode><QHI:mode>2"
  [(parallel [(set (match_operand:<QHI:MODE> 0 "nonimmediate_operand")
		   (any_fix:QHI (match_operand:SFDF 1 "gpc_reg_operand")))
	      (clobber (match_scratch:DI 2))])]
...
(define_insn_and_split "*fix_trunc<SFDF:mode><QHI:mode>2_internal"
  [(set (match_operand:<QHI:MODE> 0 "reg_or_indexed_operand" "=<SFDF:Fv>,Z")
	(fix:QHI
	 (match_operand:SFDF 1 "gpc_reg_operand" "<SFDF:Fv>,<SFDF:Fv>")))
   (clobber (match_scratch:DI 2 "=X,v"))]
  "TARGET_P9_VECTOR && TARGET_DIRECT_MOVE_64BIT
   && TARGET_VSX_SMALL_INTEGER"
  "#"
  "&& reload_completed"
  ...
  if (MEM_P (dest))
    {
      ...
      emit_insn (gen_fctiwz_<SFDF:mode> (tmp, src));
      emit_move_insn (dest, tmp2);
    }
  else
    {
      rtx dest_di = gen_rtx_REG (DImode, REGNO (dest));
      emit_insn (gen_fctiwz_<SFDF:mode> (dest_di, src));
    }
  DONE;
})
  ...
  if (MEM_P (dest))
    {
      rtx tmp = operands[2];
      rtx tmp2 = gen_rtx_REG (<QHI:MODE>mode, REGNO (tmp));
      emit_insn (gen_fctiwuz_<SFDF:mode> (tmp, src));
      emit_move_insn (dest, tmp2);
    }
  else
    {
      rtx dest_di = gen_rtx_REG (DImode, REGNO (dest));
      emit_insn (gen_fctiwuz_<SFDF:mode> (dest_di, src));
    }
  DONE;
})
