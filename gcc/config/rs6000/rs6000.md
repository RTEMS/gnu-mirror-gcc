...
(define_constants
  [(ADDR_MASK_DEFAULT		  0x0)   ; Use default addr_mask in queries
   (ADDR_MASK_VALID		  0x1)   ; Mode is valid in register.
   (ADDR_MASK_MULTIPLE		  0x2)   ; Mode takes multiple registers.
   (ADDR_MASK_INDEXED		  0x4)   ; Reg+reg addressing.
   (ADDR_MASK_OFFSET_D		 0x10)   ; Reg+offset addressing (D-form).
   (ADDR_MASK_OFFSET_DS		 0x20)   ; Reg+offset addressing (DS-form).
   (ADDR_MASK_OFFSET_DQ		 0x40)   ; Reg+offset addressing (DQ-form).
   (ADDR_MASK_OFFSET		 0x80)   ; Reg+offset addressing (any).
   (ADDR_MASK_PRE_INCDEC	0x100)   ; PRE_INC/PRE_DEC valid.
   (ADDR_MASK_PRE_MODIFY	0x200)   ; PRE_MODIFY valid.
   (ADDR_MASK_AND_M16		0x400)]) ; AND -16 addressing.
