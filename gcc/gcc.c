...
/* Explicit -dumpdir, -dumpbase and -dumpbase-ext flags.  dumpdir may
   actually be a computed -dumpdir if issue_dumpdir, and dumpbase and
   dumpbase_ext may be cleared if integrated with dumpdir or
   dropped.  */
static bool save_temps_overrides_dumpdir = false;
static bool issue_dumpdir = false;
static char *dumpdir, *dumpbase, *dumpbase_ext;
/* Basename of dump and aux outputs, computed from dumpbase (given or
   derived from output name), to override input_basename in %b et
   al.  */
static char *outbase;
int dumpdir_length = 0, outbase_length = 0;
...
/* Return the index into infiles for the single non-library input
   file, -1 if there isn't any, or -2 if there is more than one.  */
static inline int
		 ...
		 struct cl_decoded_option *decoded_options)
{
  ...
  */
  ...
  /* If dumpbase was not brought into dumpdir but we're linking, bring
     output_file into dumpdir unless dumpdir was explicitly
     specified.  */
  else if (!have_c && !explicit_dumpdir)
    {
      /* If we get here, we know dumpbase was not specified.  If it
	 was, it would have been used above, because the condition for
	 dumpbase to be used when present is broader than the
      ...
      else
	{
	 ...
	 [dir2/]foo-foo.c.*.  */
      int idxin;
      if ((idxin = single_input_file_index ()) >= 0
	  && adds_single_suffix_p (lbasename (infiles[idxin].name), obase))
	{
	  free (tofree);
	  obase = tofree = NULL;
	}
...
set_collect_gcc_options (void)
{
  ...
  if (issue_dumpdir || outbase)
	...
	obstack_grow (&collect_obstack, " ", 1);
      ...
      /* If outbase has pathname components, it will have overriden
	 dumpdir.  */
      if (dumpdir)
	{
	  q = dumpdir;
      ...
      /* For single-file links, pass the requested dumpbase or output
	 name as part of -dumpdir to lto-wrapper.  */
      if (outbase)
	{
	  q = outbase;
	  while ((p = strchr (q, '\'')) && p - outbase < outbase_length)
	    {
	      obstack_grow (&collect_obstack, q, p - q);
	      obstack_grow (&collect_obstack, "'\\''", 4);
	      q = ++p;
	    }
	  obstack_grow (&collect_obstack, q, outbase + outbase_length - q);
	  obstack_grow (&collect_obstack, "-", 1);
	}
      obstack_grow (&collect_obstack, "'", 1);
    }
  ...
  while ((c = *p++))
	...
	switch (c = *p++)
	  ...
	  case 'b':
	    ...
	    if (this_is_output_file || !outbase)
				      ...
				      const char **argv ATTRIBUTE_UNUSED)
{
  ...
  if (argbuf.length () > 0
	...
	do_spec_2 ("%{!save-temps*:%g.gkd}%{save-temps*:%B.gkd}", NULL);
      ...
      ret = concat ("-fdump-final-insns=", convert_white_space (name), NULL);
    }
...
dumps_spec_func (int argc, const char **argv ATTRIBUTE_UNUSED)
{
  ...
  p = convert_white_space (base);
  args[nargs++] = concat (" -dumpbase ", p, NULL);
  if (p != base)
    free (p);
  free (base);
  ...
  if (*ext)
    {
      char *p = convert_white_space (CONST_CAST (char *, ext));
