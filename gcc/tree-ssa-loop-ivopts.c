...
/* Return cost of computing USE's address expression by using CAND.
   ...
   address expression, respectively.  SIMPLE_INV is TRUE if AFF_INV
   is simple enough.  If AFF_INV is simple, store the loop invariant
   variables which are depended by it in INV_VARS; if AFF_INV is
   complicated, handle it as a new invariant expression and record
   it in INV_EXPR.  RATIO indicates multiple times between steps of
   USE and CAND.  If CAN_AUTOINC is nonNULL, store boolean value to
   it indicating if this is an auto-increment address.  */
		  ...
		  bool *can_autoinc, bool speed)
{
  ...
  if (simple_inv)
    simple_inv = aff_combination_simple_p (aff_inv);
  if (!aff_combination_zero_p (aff_inv))
		      ...
		      bool *can_autoinc, iv_inv_expr_ent **inv_expr)
{
  ...
  bool simple_inv = aff_combination_simple_p (&aff_inv);
