...
{
  for (is_modified_map_t::iterator i = modified_map.begin (), e = modified_map.end (); i != e; ++i)
    {
      i->second = true;
    }
}
...
TypeReconstructor::_walk_RECORD_TYPE_post (tree t)
{
  ...
  for (field_tuple_list_t::iterator i = field_tuple_list.begin (), e = field_tuple_list.end (); i != e; ++i)
    {
...
void
ExprTypeRewriter::handle_pointer_arithmetic_nonconstant (gimple *s, tree op_0,
							 tree op_1,
