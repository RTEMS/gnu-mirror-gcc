		  ...
		  if (REG_P (dst)
		      ...
		      for (;;)
			{
			  ...
			  if (REG_P (y))
				...
				{
				  /* When right shifting all the bits
				     in the shifted value are relevant.  */
				  bitmap_set_range (livenow, rn, 4);
			  ...
			  /* We might have (ashift (const_int 1) (reg...)) */
				   ...
				   && (GET_CODE (src) == LSHIFTRT || GET_CODE (src) == ASHIFTRT
				       || GET_CODE (src) == ASHIFT))
			    {
			      rtx x = XEXP (src, 1);
			      if (GET_CODE (x) == SUBREG)
...
public:
  ...
  /* opt_pass methods: */
  virtual bool gate (function *) { return !WORDS_BIG_ENDIAN && (optimize > 0); }
