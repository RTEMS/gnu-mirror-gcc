...
TESTARG(ciia, char *, int); /* { dg-error "passing argument 1 of 'ciiaF' makes pointer from integer without a cast" "" { target { ! aarch64_capability_any } } } */
...
TESTARP(ciib, char *, int); /* { dg-error "passing argument 1 of 'ciibFp.x' makes pointer from integer without a cast" "" { target { ! aarch64_capability_any } } } */
...
TESTASS(ciic, char *, int); /* { dg-error "assignment to 'char \\*' from 'int' makes pointer from integer without a cast" "" { target { ! aarch64_capability_any } } } */
                            /* { dg-error "assigning to capability 'char \\*' from incompatible type 'int'" "" { target { aarch64_capability_any } } .-1 } */
TESTINI(ciid, char *, int); /* { dg-error "initialization of 'char \\*' from 'int' makes pointer from integer without a cast" "" { target { ! aarch64_capability_any } } } */
                            /* { dg-error "initializing capability 'char \\*' with an expression of incompatible type 'int'" "" { target { aarch64_capability_any } } .-1 } */
TESTRET(ciie, char *, int); /* { dg-error "returning 'int' from a function with return type 'char \\*' makes pointer from integer without a cast" "" { target { ! aarch64_capability_any } } } */
