...
! { dg-final { scan-tree-dump-times {_gfortran_caf_is_present \((?:NON_LVALUE_EXPR <)?xx\.token>?, \(integer\(kind=4\)\) \(2 - xx\.dim\[0\]\.lbound\), &caf_ref\.[0-9]+\)|_gfortran_caf_is_present \((?:NON_LVALUE_EXPR <)?xx\.token>?, 2 - xx\.dim\[0\]\.lbound, &caf_ref\.[0-9]+\)} 10 "original" } }
