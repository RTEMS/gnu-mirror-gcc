...
bool side_effects_with_mem (const_rtx x)
{
  ...
  switch (code)
    ...
    case MEM: // We might want tu return true iff volatile or mem is a destination
...
bool is_rtx_insn_prelive(rtx_insn *insn) {
  ...
  /* TODO : What about call argumets? Accoring to the docs, for function prologue the RTX_FRAME_RELATED_P
     should return true.
  */  
  /* Callee-save restores are needed.  */
  ...
  if (GET_CODE(body) == CLOBBER) // ~/Documents/gcc/gcc/testsuite/gcc.c-torture/compile/20000605-1.c
    return true;
...
{
  if (insn->is_artificial()) // phis are never prelive, bb head + end are artificial
    return false;
...
rtl_ssa_dce_mark()
{
  ...
  while (!worklist_new.is_empty()) {
    ...
    /*
     * TODO : a phi insn might be visited more times due to having more phi nodes
