...
deletable_insn_p(rtx_insn *insn, bool fast, bitmap arg_stores)
{
  ...
  else if (DF_REF_REG(def) == pic_offset_table_rtx && REGNO(pic_offset_table_rtx) >= FIRST_PSEUDO_REGISTER)
    return false;
...
mark_insn(rtx_insn *insn, bool fast)
{
  if (!marked_insn_p(insn))
  {
    ...
    if (CALL_P(insn) && !df_in_progress && !SIBLING_CALL_P(insn) && (RTL_CONST_OR_PURE_CALL_P(insn) && !RTL_LOOPING_CONST_OR_PURE_CALL_P(insn)) && can_delete_call(insn))
...
sp_based_mem_offset(rtx_call_insn *call_insn, const_rtx mem, bool fast)
{
  ...
  if (GET_CODE(SET_SRC(set)) != PLUS || XEXP(SET_SRC(set), 0) != stack_pointer_rtx || !CONST_INT_P(XEXP(SET_SRC(set), 1)))
    return INTTYPE_MINIMUM(HOST_WIDE_INT);
...
check_argument_load(rtx *loc, void *data)
{
  ...
  FOR_EACH_SUBRTX(iter, array, *loc, NONCONST)
  {
    const_rtx mem = *iter;
    HOST_WIDE_INT size;
    if (MEM_P(mem) && MEM_SIZE_KNOWN_P(mem) && MEM_SIZE(mem).is_constant(&size))
    {
                     ...
                     bitmap arg_stores)
{
  ...
  struct check_argument_load_data data = {sp_bytes, min_sp_off, max_sp_off, call_insn, fast, false};
  ret = false;
  for (insn = PREV_INSN(call_insn); insn; insn = prev_insn)
  {
    ...
    HOST_WIDE_INT size;
    if (!MEM_SIZE_KNOWN_P(mem) || !MEM_SIZE(mem).is_constant(&size) || !check_argument_store(size, off, min_sp_off, max_sp_off, sp_bytes))
      break;
...
delete_unmarked_insns(void)
{
  ...
  if (NONDEBUG_INSN_P(insn))
  {
    ...
    if (noop_move_p(insn)
     ...
     to alter cfg.  */
        && (!cfun->can_throw_non_call_exceptions || (cfun->can_delete_dead_exceptions && can_alter_cfg) || insn_nothrow_p(insn)))
    {
                       ...
                       struct dead_debug_global *global_debug)
{
 ...
 set.  */
  ...
  if (DEBUG_INSN_P(insn))
  {
    df_ref use;
    FOR_EACH_INSN_USE(use, insn)
    if (DF_REF_REGNO(use) >= FIRST_PSEUDO_REGISTER && known_eq(GET_MODE_SIZE(GET_MODE(DF_REF_REAL_REG(use))), 2 * UNITS_PER_WORD) && !bitmap_bit_p(local_live, 2 * DF_REF_REGNO(use)) && !bitmap_bit_p(local_live, 2 * DF_REF_REGNO(use) + 1))
...
fast_dce(bool word_level)
{
  ...
  while (global_changed)
  {
    ...
    for (i = 0; i < n_blocks; i++)
    {
      ...
      if (word_level)
        local_changed = word_dce_process_block(bb, bitmap_bit_p(redo_out, index),
                                               &global_debug);
      else
...
static void
rtl_ssa_dce_mark_live(insn_info *info, vec<insn_info *> &worklist, sbitmap marked_rtx)
{
